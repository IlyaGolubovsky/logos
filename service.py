    ...
    def group_bars_by_days(bars: List[Bar], stock_name: str) -> Dict[str, TradingDay]:
        ...
        for key, bars in bar_dict.items():
            if len(bars) == 79:
                ...
                log(f"Day {key} was skipped due to wrong bars count - {len(bars)}. Required - 79")
            ...
            for _, row in group_data.iterrows():
                ...
                if key not in grouped_data:
                    grouped_data[key] = {
                        ...
                        'Close Status': row['close_type'],
                        'Open Time': row['opened_time'],
                        'Close Time': row['closed_time']
                    }

        return pd.DataFrame(grouped_data.values()).reset_index(drop=True)
